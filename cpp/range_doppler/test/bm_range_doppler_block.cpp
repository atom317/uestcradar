...
std::size_t InputElements(const Case& c) { return c.channels * c.pulses * c.samples; }
std::size_t OutputElements(const Case& c) { return c.pulses * c.samples; }
...
std::unique_ptr<fg::BlockModel> MakeBlock(const Case& c) {
    return std::unique_ptr<fg::BlockModel>(
        new fg::BlockWrapper<ProductionBlock>(
            "benchmark_range_doppler", fg::BlockTypeName{"algorithm.range_doppler"},
            MakeParams(c)));
}

std::vector<InputSample> MakeInput(const Case& c) {
    std::vector<InputSample> input(InputElements(c));
...
void RunCase(const Case& c, std::size_t minimum_works, std::size_t warmup_works,
    ...
    const std::size_t n = InputElements(c);
    const std::size_t m = OutputElements(c);
    ...
    const auto input_items = static_cast<double>(successful_works * n);
    const auto output_items = static_cast<double>(successful_works * m);
    const double input_mitems = input_items / measured_seconds / 1.0e6;
    const double output_mitems = output_items / measured_seconds / 1.0e6;
    const double input_gbps = input_items * sizeof(InputSample) / measured_seconds / 1.0e9;
    const double output_gbps = output_items * sizeof(OutputSample) / measured_seconds / 1.0e9;
    ...
    std::cout << std::fixed << std::setprecision(3)
              ...
              << "  input_Mitems/s=" << input_mitems
              << " output_Mitems/s=" << output_mitems << '\n'
              << "  input_GB/s=" << input_gbps
              << " output_GB/s=" << output_gbps
              << " aggregate_IO_GB/s=" << input_gbps + output_gbps << '\n'
    ...
    if (c.target_input_mitems_per_second) {
        std::cout << "  target_input_Mitems/s=" << *c.target_input_mitems_per_second
                  << " realtime_headroom=" << input_mitems / *c.target_input_mitems_per_second
                  << "x\n";
    }
}
...
int main(int argc, char** argv) {
    try {
        ...
        const std::vector<Case> cases{
            {"algorithm.range_doppler/1x8x64", 1, 8, 64, std::nullopt},
            {"algorithm.range_doppler/2x8x64", 2, 8, 64, std::nullopt},
            {"algorithm.range_doppler/2x32x256", 2, 32, 256, std::nullopt},
        };
        ...
        std::cerr << "bm_range_doppler_block failed: " << ex.what() << '\n';
        return 1;
    }
}
